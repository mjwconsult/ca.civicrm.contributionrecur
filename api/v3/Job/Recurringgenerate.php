...
function civicrm_api3_job_recurringgenerate($params) {
  ...
  if (empty($params['payment_processor_id'])
    ...
    Throw new CiviCRM_API3_Exception('Invalid parameters for Job.RecurringGenerate');
  }
  ...
  $settings = civicrm_api3('Setting', 'getvalue', ['name' => 'contributionrecur_settings']);

  // new contributions are either complete or left pending, default pending
  $new_contribution_status_id = empty($settings['complete']) ? 'Pending' : 'Completed';
  $new_contribution_status_id = CRM_Core_PseudoConstant::getKey('CRM_Contribute_BAO_Contribution', 'contribution_status_id', $new_contribution_status_id);
  ...
  $select = 'SELECT cr.*, count(c.id) AS installments_done, NOW() as test_now
      ...
      WHERE
        ...
        AND (cr.contribution_status_id IN (%1))
  ';

  $completedStatusID = CRM_Core_PseudoConstant::getKey('CRM_Contribute_BAO_ContributionRecur', 'contribution_status_id', 'Completed');
  $inProgressStatusID = CRM_Core_PseudoConstant::getKey('CRM_Contribute_BAO_ContributionRecur', 'contribution_status_id', 'In Progress');
  $validRecurringContributionsStatuses = [$completedStatusID, $inProgressStatusID];
  $queryParams = [1 => [implode(',', $validRecurringContributionsStatuses), 'CommaSeparatedIntegers']];
    ...
    if (isset($spec[$key])) {
      ...
      foreach ($f as $id) {
        if (!is_numeric($id) || empty($id)) {
          throw new CRM_Core_Exception('Invalid syntax: ' . $value);
        }
        else {
          $clean[] = (int) $id;
        }
  ...
  $update = 'UPDATE civicrm_contribution_recur cr
      ...
      SET
        cr.end_date = NULL
      WHERE
        cr.contribution_status_id IN (%1)
  ...
  $update = 'UPDATE civicrm_contribution_recur cr
      INNER JOIN civicrm_payment_processor pp ON cr.payment_processor_id = pp.id
      SET
        cr.contribution_status_id = %2
      WHERE
        cr.contribution_status_id = %1
        AND (cr.end_date IS NULL OR cr.end_date > NOW())'.$param_where;
  $queryParams = [
    1 => [$completedStatusID, 'Integer'],
    2 => [$inProgressStatusID, 'Integer']
  ];
  ...
  $select = "SELECT cr.*, pp.class_name, pp.is_test
      ...
      WHERE
        cr.contribution_status_id = %2
        {$param_where}
        AND cr.next_sched_contribution_date <= %3";
  // process all recurring contributions due today or earlier

  $queryParams[3] = [$dtCurrentDayEnd, 'String'];
  $dao = CRM_Core_DAO::executeQuery($select, $queryParams);
  $counter = 0;
  $output  = [];

  while ($dao->fetch()) {
    // Create all the contribution record with status = pending, so that they must be completed manually.
    ...
    if ($domemberships && !empty($contribution_template['contribution_id'])) {
      try {
        $membership_payment = civicrm_api3('MembershipPayment','getsingle', ['contribution_id' => $contribution_template['contribution_id']]);
        if (!empty($membership_payment['membership_id'])) {
          civicrm_api3('MembershipPayment','create', ['contribution_id' => $contribution_id, 'membership_id' => $membership_payment['membership_id']]);
        }
      }
      catch (Exception $e) {
