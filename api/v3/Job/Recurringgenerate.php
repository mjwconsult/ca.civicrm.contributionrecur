...
function civicrm_api3_job_recurringgenerate($params) {
  ...
  while ($dao->fetch()) {
    ...
    if ($domemberships && !empty($contribution_template['contribution_id'])) {
      try {
        $membership_payment = civicrm_api3('MembershipPayment','getsingle', array('contribution_id' => $contribution_template['contribution_id']));
        if (!empty($membership_payment['membership_id'])) {
          civicrm_api3('MembershipPayment','create', array('contribution_id' => $contribution_id, 'membership_id' => $membership_payment['membership_id']));
        }
      }
      catch (Exception $e) {
